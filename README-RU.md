...
**Порты-исключения 🚫**
  * `match-set skeen_exclude_port dst ACCEPT`
  * **Суть:** Если ты в `skeen.json` указал порты, которые не надо проксировать (или наоборот, указаны только конкретные) - здесь трафик либо улетает напрямую, либо идет дальше на проверку.
