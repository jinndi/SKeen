...
| Команда | WEB CLI поддержка | FW Only поддержка | Описание |
| :--- | :---: | :---: | :--- |
...
| `check` | ✓ | ✓ | Проверяет текущую конфигурацию Sing-box (если работа без `firewall.only`) на синтаксические и логические ошибки + валидность `skeen.json` |
...
| `sync`² | ✓ | ✓ | Синхронизирует конфигурацию Sing-box (либо другую в режиме `firewall.only`) по пути, указанному в `sing_config.path` файла `skeen.json` |
