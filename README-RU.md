...
Если вы запретили доступ из интернета к своему субдомену в разделе «Доменное имя» панели управления, то для доступа к **KeenDNS** из локальной сети в режиме **TProxy** вам необходимо настроить в конфигурации **sing-box** DNS-сервер типа `hosts` (пример настройки см. в файле `examples/keendns-tproxy-local-access.json`). Если же доступ из интернета разрешен, дополнительные настройки не требуются.
