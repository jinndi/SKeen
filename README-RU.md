...
Используется цепочка PREROUTING в таблице `nat` под именем **skeen**:

Вход в данную цепочку может осуществляться по `fwmark` политики роутера. Она содержит следующий порядок правил:

* **ACCEPT** - обход портов, заданных в `skeen.json` (только если не включена опция работы на выбранных портах).
* **ACCEPT** - обход локальных, зарезервированных и пользовательских адресов.
* **REDIRECT** - переадресация TCP-трафика на порт `redirect` Sing-Box, включая только те порты, что заданы в настройках `skeen.json`.

---

### TProxy - задействован в режимах `tproxy` (TCP & UDP) и `hybrid` (UDP), а также для проксирования роутера

Используется цепочка PREROUTING в таблице `mangle` под именем **skeen**:

Вход в данную цепочку может осуществляться по `fwmark` политики роутера. Содержит следующий порядок правил:

* **DNS TPROXY** - TCP/UDP редирект 53 порта на порт TProxy Sing-Box (опционально, иначе - ACCEPT).
* **ACCEPT** - обход портов, заданных в `skeen.json` (только если не включена опция работы на выбранных портах).
* **ACCEPT** - обход локальных, зарезервированных и пользовательских адресов.
* **TCP MARK + ACCEPT SOCKET** - быстрый путь для уже установленных прозрачных сокетов (socket transparent).
* **TPROXY** - направление оставшегося TCP/UDP трафика в порт TProxy Sing-Box, либо только портов, заданных в настройках `skeen.json`.
...
Вход в данные цепочки возможен только для процессов, не состоящих в группе `skeen` (для исключения самозацикливания прокси). Содержит следующий порядок правил:

1. Режим `redirect`, таблица `nat` в `OUTPUT` с именем `skeen_mask`: аналогично правилам Redirect цепочки `skeen`.
2. Режим `tproxy`, таблица `mangle` в `OUTPUT` с именем `skeen_mask`: аналогично правилам TProxy цепочки, за исключением отсутствия DNS-правил и правил прямого направления трафика в Sing-Box. Вместо этого в конце идет:
