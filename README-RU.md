...
**Исключения (ipset) ⚡️**
  * `match-set skeen_exclude_port dst ACCEPT` - игнорим пользовательские порты.
  * `match-set skeen_exclude_net4 dst ACCEPT` - не трогаем локалку, зарезервированные сети и кастомные подсети из `skeen.json`.
...
**Захват остального трафика (Catch-all) 🕸**
  * `connmark match 0x112 MARK` - метим всё, что не попало в списки выше.
...
Это крайне не рекомендуемое действие. С точки зрения производителя, это отличный способ быстрее износить память вашего устройства для его скорейшей замены на новое. В SKeen такая опция не появится никогда. Вам как минимум необходим роутер с USB-портом и накопителем объемом более 256 МБ.
