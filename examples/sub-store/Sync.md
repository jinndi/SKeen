...
2. Настраиваем `Sub-Store`:
  ...
  * **``Профиль``**: Во-первых, переключитесь на русский язык (в меню слева самая последняя вкладка — **``Профиль``**; в правом верхнем углу будет значок переключения). Там же настройте GitHub для синхронизации всех настроек с Gist: введите имя пользователя и токен, создав его по адресу https://github.com/settings/tokens, с правами на чтение и запись в Gists.
  ...
  > [!NOTE]
  > Если подписка требует HWID и другие параметры - получите их через команду `skeen headers` и добавьте в конец вашей ссылки на подписку. При необходимости измените имя клиента в поле User-Agent интерфейса Sub-Store.
...
```jsonc
{
  ...
  "route": {
    "rules": [
      { "network": "icmp", "action": "resolve" },
      { "network": "icmp", "action": "bypass", "outbound": "🔌 DIRECT" },
  ...
  "services": [
    {
      ...
      "access_control_allow_origin": [
        "http://sing-box-dashboard.sagernet.org", "https://sing-box-dashboard.sagernet.org",
        "http://sdash.u1.pw", "https://sdash.u1.pw"
      ],
  ...
  "experimental": {
    ...
    "clash_api": {
      ...
      "access_control_allow_origin": [
        "http://board.zash.run.place", "https://board.zash.run.place",
        "https://d.metacubex.one", "https://metacubexd.pages.dev"
      ],
      "access_control_allow_private_network": true
    },
    "debug": {
