...
> ⚠️ **ВАЖНО:** Эта инструкция предназначена исключительно для обучения и экспериментов. Нет ничего бессмысленнее, чем настраивать на домашнем роутере сервер для раздачи интернета внешним клиентам. Во-первых, если вам нужен доступ к локальным файлам, в Keenetic/Netcraze для этого уже есть встроенные решения (например, WebDAV). Во-вторых - и это аксиома - для качественного и быстрого доступа в интернет внешние устройства должны подключаться через клиентские приложения напрямую к удаленным прокси-узлам, а не гонять трафик через ваш домашний роутер, и быть при этом со своей собственной настроенной маршрутизацией.
