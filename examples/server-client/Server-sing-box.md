...
```jsonc
{
  ...
  /// Блок настройки конечных точек (endpoints)
  "endpoints": [
    // 1. Tailscale как выходной узел c SSH сервером
    ...
    // Настройка на роутере в файле endpoints.jsonc
    ...
    // 2. MASQUE Server - представляет собой IP-прокси, работающий поверх HTTP-сервера (RFC 9484, CONNECT-IP).
    // Термин "MASQUE VPN" обычно относится к CONNECT-IP, а не к комбинации CONNECT и CONNECT-UDP,
    // которая является усовершенствованием HTTP-проксирования. CONNECT-UDP и CONNECT-IP вместе известны как MASQUE в IETF;
    // традиционный метод CONNECT для TCP-проксирования сам по себе не является протоколом MASQUE.
    // Требование: Sing-box 1.15.0-alpha.7 + версии.
    {
      "type": "masque-server",
      "tag": "masque-server-ep",
      "listen": "::",
      "listen_port": 8443,
      "users": [
        {
      ...
      "address": [
        "10.0.0.1/24" // любой часный CIDR, можно добавить и IPv6 если нужен.
      ],
      "tls": {
        "enabled": true,
  ...
  /// Примеры конфигурации на стороне клиента: смотрите в файле `outbounds.jsonc`
  "inbounds": [
...
Именно эту строку необходимо скопировать и вставить в конфигурационный файл клиента в значение ключа `certificate_public_key_sha256` настройки `tls`, примеры смотрите в файле `outbounds.jsonc`.

> Как это делатать автоматически для ваших подписок в Sub-Store смотрите в файле [sub-store/Examples.md](../sub-store/Examples.md)
...
**Начиная с версии sing-box 1.15.0-alpha.7** появился более строгий метод проверки серверного и клиентского сертификата (`certificate_sha256` и `client_certificate_sha256` соответсвенно), где вычисление хеша идет по всему сертификату, закодированному в формате DER, поэтому он изменяется при каждом его обновлении, даже если ключ остается тем же.
Используйте `certificate_public_key_sha256` когда нужно закрепить только ключ.

Для генерации хеша SHA-256 для сертификата используйте следующие команды:

```bash
# Для файла сертификата
openssl x509 -in /root/sing-box/certself/self.crt -outform der | openssl dgst -sha256 -binary | openssl enc -base64

# Для сертификата с удаленного сервера
...
Для клиентов указывайте полученный результат в ключе `tls.certificate_sha256`, а для обратной проверки сервером клиентского сертификата - `tls.client_certificate_sha256`.
