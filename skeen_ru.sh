...
format_config() {
  if get_singbox_config && [ -f "$SINGBOX_BIN" ]; then
    ...
    # shellcheck disable=SC2086
    if $SINGBOX_BIN format -w -c $SINGBOX_CONFIG; then
...
sync_config() {
  ...
  [ -f "$SINGBOX_CONFIG" ] && cp -f "$SINGBOX_CONFIG" "${SINGBOX_CONFIG}.bak"
