...
// https://github.com/jinndi/SKeen/blob/main/docs/CONFIGURATION-RU.md
{
  "auto_start": {
    "enabled": 1,
    "delay": 0
...
gen_headers() {
  ...
  cyan "Сгенерированные случайные параметры устройства:"
  for k in x-hwid x-device-os x-ver-os x-device-model; do printf '  %s: ' "$k"; green "$(jsonfilter -s "$json" -e "@['$k']")"; done
  printf '\n'; cyan "Sub-Store: Добавьте в конец ссылки подписки параметр:"; green "$(printf '%s' "#headers=$json" | sed 's/ /%20/g')"
}
