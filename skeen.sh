  ...
  case "$sing_path" in
    ...
    *)
      if kill -0 "$sing_pid" 2>/dev/null; then
        echomsg "Terminating $SINGBOX_NAME (old process PID $sing_pid)..."
...
start_singbox() {
  ...
  get_sing_args_config
  # shellcheck disable=SC2086
  start-stop-daemon -S -b -x $SINGBOX_BIN -c root:$SKEEN_PROC -- $SINGBOX_ARGS
  status_start=$?
