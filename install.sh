...
apply_sysctl_network_tuning(){
  ...
  sysctl -w net.core.rmem_max=2097152        >/dev/null 2>&1  # Max TCP/UDP receive buffer
  sysctl -w net.core.wmem_max=2097152        >/dev/null 2>&1  # Max TCP/UDP send buffer
  sysctl -w net.core.rmem_default=229376     >/dev/null 2>&1  # Default receive buffer
  sysctl -w net.core.wmem_default=229376     >/dev/null 2>&1  # Default send buffer

  # Interface Queues
  sysctl -w net.core.netdev_max_backlog=2048 >/dev/null 2>&1  # Max packets queued on interface
  sysctl -w net.core.somaxconn=256           >/dev/null 2>&1  # Max pending TCP connections

  # IPv4 Forwarding & TProxy Support
  sysctl -w net.ipv4.ip_forward=1                  >/dev/null 2>&1  # Enable IPv4 routing
  sysctl -w net.ipv4.conf.all.src_valid_mark=0     >/dev/null 2>&1  # Accept TProxy marked packets
  sysctl -w net.ipv4.conf.lo.route_localnet=1      >/dev/null 2>&1  # Allow lo local routing (TProxy)
  sysctl -w net.ipv4.conf.all.send_redirects=0     >/dev/null 2>&1  # Disable ICMP redirects globally
  sysctl -w net.ipv4.conf.default.send_redirects=0 >/dev/null 2>&1  # Disable ICMP redirects by default

  # Connection Tracking
  sysctl -w net.netfilter.nf_conntrack_max=40960                   >/dev/null 2>&1  # Max tracked connections
  sysctl -w net.netfilter.nf_conntrack_tcp_timeout_established=600 >/dev/null 2>&1  # TCP established timeout

  # TCP/UDP Memory & Buffers
  sysctl -w net.ipv4.tcp_mem="8192 16384 32768"    >/dev/null 2>&1  # TCP memory thresholds
  sysctl -w net.ipv4.udp_mem="8192 16384 32768"    >/dev/null 2>&1  # UDP memory thresholds
  sysctl -w net.ipv4.tcp_rmem="4096 87380 4194304" >/dev/null 2>&1  # TCP per-socket read buffer min/def/max
  sysctl -w net.ipv4.tcp_wmem="4096 65536 4194304" >/dev/null 2>&1  # TCP per-socket write buffer min/def/max
  sysctl -w net.ipv4.udp_rmem_min=16384            >/dev/null 2>&1  # Min UDP receive buffer
  sysctl -w net.ipv4.udp_wmem_min=16384            >/dev/null 2>&1  # Min UDP send buffer
  sysctl -w net.ipv4.tcp_limit_output_bytes=262144 >/dev/null 2>&1  # Limit per-socket output burst

  # TCP Behavior / Optimizations
  sysctl -w net.ipv4.tcp_syncookies=1        >/dev/null 2>&1  # Enable SYN cookies (SYN flood protection)
  sysctl -w net.ipv4.tcp_tw_reuse=1          >/dev/null 2>&1  # Allow reuse of TIME_WAIT sockets
  sysctl -w net.ipv4.tcp_fin_timeout=30      >/dev/null 2>&1  # Shorten FIN timeout
  sysctl -w net.ipv4.tcp_keepalive_time=600  >/dev/null 2>&1  # TCP keepalive interval
  sysctl -w net.ipv4.tcp_keepalive_probes=5  >/dev/null 2>&1  # Keepalive probes count
  sysctl -w net.ipv4.tcp_keepalive_intvl=10  >/dev/null 2>&1  # Keepalive interval between probes
  sysctl -w net.ipv4.tcp_timestamps=0        >/dev/null 2>&1  # Disable TCP timestamps for performance
  sysctl -w net.ipv4.tcp_sack=1              >/dev/null 2>&1  # Enable selective ACKs
  sysctl -w net.ipv4.tcp_max_syn_backlog=512 >/dev/null 2>&1  # Max SYN backlog
  sysctl -w net.ipv4.tcp_max_tw_buckets=8192 >/dev/null 2>&1  # Max TIME_WAIT sockets
  sysctl -w net.ipv4.tcp_fastopen=3          >/dev/null 2>&1  # Enable TCP Fast Open
  sysctl -w net.ipv4.tcp_mtu_probing=0       >/dev/null 2>&1  # Disable TCP MTU probing

  # Local Ports
  sysctl -w net.ipv4.ip_local_port_range="10000 60001" >/dev/null 2>&1  # Set ephemeral port range

  # IPv6 Forwarding
  if [ -f /proc/net/if_inet6 ]; then
    sysctl -w net.ipv6.conf.all.forwarding=1     >/dev/null 2>&1  # Enable IPv6 routing globally
    sysctl -w net.ipv6.conf.default.forwarding=1 >/dev/null 2>&1  # Enable IPv6 routing by default
  fi
}
